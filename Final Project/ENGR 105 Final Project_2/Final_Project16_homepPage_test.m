...
home = figure('visible','on');
set(home,'visible','on');
%homeScreen = axes('units','normalized','position',[0,0,1,1]);
...
g3_image = imagesc(startScreenImage);
% set the visibility of the axes to invisible
set(g3,'handlevisibility','off','visible','on')
% denote the size of the map so that it is the same size for all computers
...
%help menus call back
helpScreen = figure(3);
set(helpScreen,'visible','off');

g4 = axes('units','normalized', ...
            'position',[0,0,1,1]);
% turn tick marks off
set(gca,'xtick',[])
set(gca,'ytick',[])
...
% graphics for health bar (damage is encoded in guidata)
boundaryHealth_help = rectangle('Position',[900 15 0 20], 'LineWidth', 3);
health_help = rectangle('Position',[900 15 500 20], 'FaceColor', 'green');
damage_help = rectangle('Position', [900 15 0 20], 'FaceColor', 'red');
...
helpButton.Callback = {@helpMenu,h1,helpScreen};

% move the balloons
t_redBalloons = timer("TimerFcn",{@razaq_moveBalloon6,h1,redBalloonVec,0,bananaScore,pointScore,damage},...
    "StopFcn",{@stopTimer},"Period",0.1,"ExecutionMode","fixedRate", ...
     "TasksToExecute",800);
 
startButton.Callback = {@startGame,h1,g3,startButton,helpButton,t_redBalloons};




% resetButton = uicontrol('Style','pushbutton','Units','Normalized','Position',...
%     [0,0.4,0.2,0.2],'String','New Game');
% set(resetButton,'visible','off')
% resetButton.Callback = {@resetGame,h1};

%         t_blueBalloons = timer("TimerFcn",{@moveBalloon,h1,blueBalloonVec,numRed,damage},...
%             "StopFcn", {@stopTimer},"Period",0.01,"ExecutionMode","fixedRate", ...
%              "TasksToExecute",800,"StartDelay",10);
%         t_greenBalloons = timer("TimerFcn",{@moveBalloon,h1,greenBalloonVec,numRed+numBlue,damage},...
%             "StopFcn", {@stopTimer},"Period",0.01,"ExecutionMode","fixedRate", ...
%              "TasksToExecute",900,"StartDelay",20);

%,t_blueBalloons,t_greenBalloons])
