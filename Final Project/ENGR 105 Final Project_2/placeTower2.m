function placeTower2(h1,~,towerBank)
    ...
    if handles.bananas >= 2000
        handles.ind = [];
        % Lose some money upon placing the tower (amount used to buy that
        % tower)
        handles.bananas = handles.bananas - 2000;
        guidata(h1,handles);
        
        % Display text that indicates to user how many bananas they lost upon placing 
        loss = text(tower.XData(1)+width,tower.YData(1)+height,"-2000",'Color','r','FontWeight','bold');
        pause(0.25);
        delete(loss);        
        
    else
        % Display a text if the user tries placing a tower without enough
        % banana currency
        rich = text(tower.XData(1)+width,tower.YData(1)+height,"Not enough bananas!",'Color','r','FontWeight','bold');
        pause(0.5);
        delete(rich);
        return
    end
