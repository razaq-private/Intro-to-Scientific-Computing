...
while gameStillGoing < 200
    ...
    for i = balloons
        ...
        if balloonStep(i) < 65 && balloonStep(i) ~=0
            balloonObj(i).XData = balloonObj(i).XData+5;
        elseif balloonStep(i) >= 65 && balloonStep(i) < 100  
            balloonObj(i).YData = balloonObj(i).YData+5;
        elseif balloonStep(i) >= 100 && balloonStep(i) < 173
            balloonObj(i).XData = balloonObj(i).XData+5;
        % make the balloon move right if it hasn't hit the start pos yet
        elseif balloonStep(i) == 0
            balloonObj(i).XData = balloonObj(i).XData+5;
        end
        ...
        if balloonObj(i).XData == 6 && balloonObj(i).YData == 55
            endPos(i) = 1;
        end
        ...
        if endPos(i) == 1
            ...
            if damage.Position(3) == health.Position(3)
                gameStillGoind = false;
            end 
        end 
    end
